...
describe('Japanesify', () => {
  ...
  test('can convert roman alphabet to hiragana and back', async () => {
    ...
    let enableToggleText = await extension.$eval(
    ...
    enableToggleText = await extension.$eval(
      '[data-testid="enable-button"]',
      (el) => el.textContent
    );
    ...
    const originalImage =
      await wikiPage.screenshot(/*{ path: path.join(__dirname, 'original.png')}*/);
    let body = await wikiPage.$eval(
      'body',
    ...
    let んswitchStatus = await extension.$eval(
    ...
    んswitchStatus = await extension.$eval(
      '[data-testid="ん-switch"]',
      (input) => {
        return (input as HTMLInputElement).checked;
      }
    ...
    await wikiPage2.goto(
      'https://en.wikipedia.org/wiki/Empress_Xiaocigao_(Qing_dynasty)'
    );
  ...
  test('can convert roman alphabet: a, ka, sa, ta, na, ha, ma, ya, ra, wa to hiragana: あ, か, さ, た, な, は, ま, や, ら, わ and back', async () => {
    ...
    await extension.bringToFront();
    let あswitchStatus = await extension.$eval(
    ...
    あswitchStatus = await extension.$eval(
      '[data-testid="あ-switch"]',
      (input) => {
        return (input as HTMLInputElement).checked;
      }
  ...
  test('can convert roman alphabet: i, ki, shi, chi, ni, hi, mi, ri to hiragana: い, き, し, ち, に, ひ, み, り and back', async () => {
    ...
    // He goes back and notices none of the conversions are selected, he then decides to enable 'あ' character.
    await extension.bringToFront();
    let いswitchStatus = await extension.$eval(
    ...
    いswitchStatus = await extension.$eval(
      '[data-testid="い-switch"]',
      (input) => {
        return (input as HTMLInputElement).checked;
      }
  ...
  test('can convert roman alphabet: u, ku, su, tsu, nu, fu, mu, ru, yu to hiragana: う, く, す, つ, ぬ, ふ, む, る, ゆ and back', async () => {
    ...
    await extension.bringToFront();
    let うswitchStatus = await extension.$eval(
    ...
    うswitchStatus = await extension.$eval(
      '[data-testid="う-switch"]',
      (input) => {
        return (input as HTMLInputElement).checked;
      }
  ...
  test('can convert roman alphabet: e, ke, se, te, ne, he, me, re to hiragana: え, け, せ, て, ね, へ, め, れ and back', async () => {
    ...
    // He goes back and notices none of the conversions are selected, he then decides to enable 'う' character.
    await extension.bringToFront();
    let えswitchStatus = await extension.$eval(
    ...
    えswitchStatus = await extension.$eval(
      '[data-testid="え-switch"]',
      (input) => {
        return (input as HTMLInputElement).checked;
      }
  ...
  test('can convert roman alphabet: o, ko, so, to, no, ho, mo, ro, yo, wo to hiragana: お, こ, そ, と, の, ほ, も, ろ, よ, を and back', async () => {
    ...
    await extension.bringToFront();
    let おswitchStatus = await extension.$eval(
    ...
    おswitchStatus = await extension.$eval(
      '[data-testid="お-switch"]',
      (input) => {
        return (input as HTMLInputElement).checked;
      }
